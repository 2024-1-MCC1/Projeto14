...
public class PlantaScript : MonoBehaviour
{
    ...
    void PegarObjeto()
    {
        ...
        if (distanciaMaoObjeto <= raio && Input.GetKeyDown(KeyCode.E) && playerScript.maosOcupadas == false && objetoNoDrop == false)
        {
            objetoNaMao = true;
            objetoNoSpawn = false;
    ...
    void LargarObjetoNoDrop()
    {
        for (int i = 0; i < distanciasDrops.Length; i++)
        {
            ...
            if (distanciasDrops[i] <= dropsScripts[i].raio && Input.GetKeyDown(KeyCode.E) && objetoNaMao == true && dropsScripts[i].ocupado == false)
            {
                objetoNaMao = false;
    ...
    void SerRegada()
    {
        ...
        if (distanciaDoRegador <= raio && Input.GetKeyDown(KeyCode.E) && objetoNoDrop == true)
        {
            plantaRegada = true;
        }
