...
public class FerramentaDeJardinagemScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (distanciaMaoObjeto <= raio && Input.GetKeyDown(KeyCode.E) && !playerScript.maosOcupadas)
        {
            objetoNaMao = true;
        ...
        if (transform.position != origem.transform.position && Input.GetKeyDown(KeyCode.F))
        {
            objetoNaMao = false;
